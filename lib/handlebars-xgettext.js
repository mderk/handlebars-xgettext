      ...
      for (var msgid in strings) {
        if (strings.hasOwnProperty(msgid)) {
          ...
          if (strings[msgid].comment) {
            context[msgid].comments.extracted = (context[msgid].comments.extracted || '')
            .split('\n')
            .concat(strings[msgid].comment)
            .join('\n')
            .trim('\n');
          }
  ...
  function output () {
    if (callback) {
      if (Object.keys(context).length > 0 || options['force-po']) {
        po = gt.po.compile({
          charset: options['to-code'],
          headers: {
            'content-type': 'text/plain; charset=' + options['to-code'],
            'plural-forms': options['plural-forms']
          },
          translations: {
            '': context
          }
